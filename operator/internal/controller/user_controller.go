...
// +kubebuilder:rbac:groups=keycloak.opendefense.cloud,resources=users,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=keycloak.opendefense.cloud,resources=users/status,verbs=get;update;patch
// +kubebuilder:rbac:groups=keycloak.opendefense.cloud,resources=users/finalizers,verbs=update
// +kubebuilder:rbac:groups=keycloak.opendefense.cloud,resources=realms,verbs=get;update

func (r *UserReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := ctrl.LoggerFrom(ctx)

	var obj v1alpha1.User
	...
	if !obj.DeletionTimestamp.IsZero() {
		if controllerutil.ContainsFinalizer(&obj, finalizer) {
			...
			if safe {
				log.Info("User was successfully purged from Keycloak, removing finalizer", "username", obj.Spec.Username)
				r.Recorder.Eventf(&obj, corev1.EventTypeNormal, "SafeDelete", "Successfully purged User '%s' from Keycloak", obj.Spec.Username)
	...
	if err := r.sync(ctx, &obj); err != nil {
		log.Error(err, "sync failed", "username", obj.Spec.Username)
		r.Recorder.Eventf(&obj, corev1.EventTypeWarning, "SyncFailed", "Failed to delegate sync: %v", err)
		err2 := UpdateStatusWithRetry(ctx, r.Client, req.NamespacedName, &obj, func(latest *v1alpha1.User) {
...
func (r *UserReconciler) sync(ctx context.Context, obj *v1alpha1.User) error {
	...
	setReady(&obj.Status.CommonStatus, obj.Spec.Username, "Delegated to Realm Sync")
