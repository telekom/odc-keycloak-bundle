...
const (
	// LabelApp is the label applied to all config-cli Jobs and Secrets managed by this operator.
...
func (r *JobRunner) discoverPullSecrets(ctx context.Context, namespace string) []corev1.LocalObjectReference {
	...
	if podName == "" {
		log.FromContext(ctx).Info("POD_NAME not set; imagePullSecrets discovery disabled — " +
			"ensure the Helm chart injects POD_NAME via fieldRef")
		return nil
	}
	...
	var pod corev1.Pod
	if err := reader.Get(ctx, types.NamespacedName{Name: podName, Namespace: namespace}, &pod); err != nil {
		log.FromContext(ctx).V(1).Info("Could not read own pod for imagePullSecrets discovery", "error", err)
		return nil
	}
...
// It returns the name of the newly created Job, or an empty string when the sync was a no-op.
func (r *JobRunner) SyncRealm(ctx context.Context, realm *v1alpha1.Realm, export *RealmExport, scheme *runtime.Scheme) (string, error) {
	...
	// Fetch the current secret for drift detection before deciding whether to sync.
	var currentSecret corev1.Secret
	getErr := r.Client.Get(ctx, types.NamespacedName{Name: secretName, Namespace: namespace}, &currentSecret)
	if getErr != nil && !apierrors.IsNotFound(getErr) {
		return "", fmt.Errorf("getting config secret %q: %w", secretName, getErr)
	}
	if getErr == nil {
		// Compare existing payload. If identical, check for periodic drift timeout.
		if string(currentSecret.Data["realm.json"]) == string(payload) {
			var lastSyncTime time.Time
			if tStr := currentSecret.Annotations["last-sync"]; tStr != "" {
	...
	_, err = controllerutil.CreateOrUpdate(ctx, r.Client, configSecret, func() error {
		...
		configSecret.Labels["app"] = labelApp
		configSecret.Annotations["last-sync"] = time.Now().Format(time.RFC3339)
		if configSecret.Data == nil {
			configSecret.Data = make(map[string][]byte)
		}
		configSecret.Data["realm.json"] = payload
		return controllerutil.SetControllerReference(realm, configSecret, scheme)
	})
	if err != nil {
		return "", fmt.Errorf("syncing config secret %q: %w", secretName, err)
	}
	...
	operatorPodNamespace := os.Getenv("POD_NAMESPACE")
	if operatorPodNamespace == "" {
		operatorPodNamespace = os.Getenv("WATCH_NAMESPACE")
	}
	if operatorPodNamespace == "" {
		operatorPodNamespace = namespace
	}
		...
		Spec: batchv1.JobSpec{
			...
			Template: corev1.PodTemplateSpec{
				...
				Spec: corev1.PodSpec{
					...
					ServiceAccountName:           os.Getenv("CONFIG_CLI_SA_NAME"),
	...
	logger.Info("Spawned keycloak-config-cli Job", "job", job.Name)
	return job.Name, nil
}
